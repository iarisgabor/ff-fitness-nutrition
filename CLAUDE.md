...
| aplicația **nativă** a Pulsului (Kotlin/Compose, „Pulsul nativ"), `/api/app`, token Bearer | `personal/pulsul-duminicii/android-nativ/README.md` + `src/appApi.js` |
...
- `android-nativ/` — aplicația Android **nativă** (Kotlin + Compose, pachet separat), aceleași ecrane, date prin `src/appApi.js` (`/api/app/…`, exact `PAYLOAD`-ul fiecărei pagini)
...
telefon, service worker cu acces offline la paginile deja deschise (șterse la Ieșire), fișiere
statice în `public/` servite prin `[assets]` — tot fără build step. Regulile 21–30 din
`personal/pulsul-duminicii/CLAUDE.md`.
